...
| TUI (Rust/ratatui) | 235 | ✅ | 用户端/城主端/世界广场/私聊终端 |
| CLI (Rust) | 148 | ✅ | 119 unit + 24 integration + 5 额外测试；login/nickname/directory/snapshot/moderate/admin 命令 |
| H5 Web Shell (JS) | 1419 | — | 7 页面, 场景交互, admin-ds 写操作护栏, Gateway 真源投影, Gateway fail-closed/会话失效/合法空投影在线, app.js 7,529 行 |
| crypto-mls | 24 | ✅ | AES-256-GCM + HKDF 前向安全 |
| ai-sidecar | 7 | ✅ | HTTP AI 助手 + 流式 |
| chat-core | 20 | ✅ | 合同定义, SceneImageLayer day/night |
| chat-storage | 18 | ✅ | 文件存储, 原子写 |
| **总计** | **2,194** | **零警告** | 本轮逐包测试通过；完整 release gate 已包含 provider federation |

最近更新（8-13）：
- 🛡️ H5 + admin-ds Gateway 会话失效闭环：401/403 清除过期 Bearer、降为访客并保留重新登录提示；真实双浏览器分别覆盖 H5 直接打开登录 overlay 与 admin-ds HUD 重新登录入口，Web 1419、layout、realness 与完整 release gate 全绿
- 🔑 H5 Gateway 会话失效单元与运行时回归：shell state 401/403 阻断 stale-session 轮询/SSE；真实 H5 浏览器回归已补齐，H5 当前 Web 入口仍以 Gateway 合同为真源
...
- 🧩 H5 世界/治理/居民/房间/会话摘要/线程状态表面职责下沉；Gateway 323、TUI 235 与完整 release gate 全绿
