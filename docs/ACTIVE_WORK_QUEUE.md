...
| 项目 | 状态 | 说明 |
| --- | --- | --- |
| 发布锚点（候选） | 已就绪 | 最新候选发布 SHA `710bdf459258d6c5f60787b8a3d85f26e42d5926`（release run 33915810037，verify/aarch64/x86_64 三 job 绿；含 CLI 通道推送覆盖）；制品目录 `/Volumes/AJW-Data/Projects/lobster-chat-release-710bdf4-run33915810037`（双架构）。注：`05e4e69` 的制品已被 710bdf4 取代（后者多了 CLI 推送覆盖的 Gateway 变更）。 |
...
| 下一步 | 等授权 | 用户授权后按 DEPLOYMENT §2 只读预检 → §3 备份+安装 → §4 公网追溯（EXPECT_RELEASE_GIT_SHA=710bdf4…）→ §5 真实 OTP/双居民验收 → §6 回滚判定。目标机 x86_64，使用 x86_64 制品。 |
...
| 下一步（本项未完） | 收口 | 第二步（`ea070da`）字节级压实 + 存储层压测（`331ef7a`）已完成；**Gateway 层真实路径压测已完成（7d600d2）**：`bench_gateway_http_append_journal_throughput` 经真实 loopback HTTP 栈（tiny_http→dispatch→校验→journal→conversations 持久化）投递 2,000 条，实测 ~37.5ms/条（限流 30 条/分/发送者，70 个轮换发送者模拟真实用户；双 fsync + HTTP 栈 + SSE 通知计入）；**重启恢复 2,000 条 快照+journal 重放仅 8.6ms**，全部还原。剩余 `recent_messages` 内存快照化属 R1 不混入。生产部署随下一发布批次，需单独授权。 |
...
| 项目 | 状态 | 说明 |
| --- | --- | --- |
| 现网发现 | 已确认 | 无 Bearer、无 `resident_id` 访问公网 `/v1/shell/state` 与 `/v1/shell/events?wait_ms=0` 均返回 200，旧部署响应含 `dm:` 私聊投影；`/v1/admin/summary` 仍为 401。 |
...
| GitHub/制品 | 已完成 | 当前可部署制品锚点为 `6c0dc6a5c5429b54f15bc0c7c403e0e393f0488d`；release `31640540602` 的 verify、x86_64、aarch64 三 job 全绿；双架构制品已下载到 `/Volumes/AJW-Data/Projects/lobster-chat-release-6c0dc6a-run31640540602`，SHA/manifest/ELF/敏感文件名校验全部通过。该制品之后仅允许文档/交接更新，若修改 Gateway/Web 源码必须重新 release。 |
